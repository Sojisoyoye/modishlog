  ...
  template: `
    <div>
      <div class="mb-6">
        ...
        <p class="mt-1 text-sm text-gray-500">Business insights and financial summaries</p>
      </div>
      ...
      <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        @for (card of reportCards; track card.route) {
          <a
            ...
            class="group flex flex-col rounded-xl border border-gray-100 bg-white p-6 shadow-sm transition-shadow hover:border-emerald-200 hover:shadow-md cursor-pointer"
          >
            <div class="mb-4 flex h-12 w-12 items-center justify-center rounded-xl" [class]="card.color">
              <i [class]="'pi ' + card.icon + ' text-xl'"></i>
            </div>
            <h3 class="text-lg font-semibold text-gray-900 group-hover:text-emerald-700">{{ card.title }}</h3>
            <p class="mt-1 text-sm text-gray-500">{{ card.description }}</p>
            <div class="mt-4 flex items-center gap-1 text-sm font-medium text-emerald-600">
